...
M_bpsk = 2;     M_qpsk_4 = 4;       M_qpsk_8 = 8;                        % No of symbols for BPSK modulation
M_qam_16 = 16;                                                  % No of symbols for PSK modulation
...
%const = qammod([0:M_qam_16-1],M_qam_16);
...
TxCy_qpsk_8 = [IFFT_Data_qpsk_8((128-Length_of_cyclic_prefix+1):128,:); IFFT_Data_qpsk_8];       % Cyclic prefix

[r c] = size(TxCy_bpsk);

Tx_Data_bpsk = TxCy_bpsk;
Tx_Data_qpsk_4 = TxCy_qpsk_4;
Tx_Data_qpsk_8 = TxCy_qpsk_8;
...
for i = 1:length(snr)
    for j = 1:c                                         % Transmit frame by frame
        ...
        hx_qpsk_8 = filter(channel,Tx_Data_qpsk_8(:,j).');                  % Pass through Rayleigh channel
...
%% Receiver
        ...
        Rx_qpsk_8 = y_qpsk_8(Length_of_cyclic_prefix+1:r);                                % Removal of cyclic prefix 
        ...
        FFT_Data_qpsk_8 = (sqrt(120)/128)*fft(Rx_qpsk_8,No_of_subcarriers)./H(j,:);   % Frequency Domain Equalization
