...
%---- Equalizer settings ----%
eq_select = 1; % selection of equalizer type 1: one tap, 
...
for i=1:num_frames
%     C = -1*(real(scp_preamble2(:,i))./imag(scp_preamble2(:,i)));
% %     ch_resp_est(:,i) = scp_preamble(:,i)./(preamble(:,1)*sumfactor);
%     Hr = (real(scp_preamble(:,i))+C.*imag(scp_preamble(:,i)))./((((-1).^(0:M-1)).').*(1+C.^2));
%     ch_resp_est(:,i)=Hr+j*(C.*Hr);
%     ch_resp_est(:,i) = (jp_scp_preamble(:,i))./(preamble(:,1)*sumfactor);
    for ii=1:M
        if mod(ii-1,2) == 0 % even subcarrier
